...
///
/// fn main() {
///     let fut = tokio_fs::xattr::get(".", "user.myattr1").map(|val| {
///         println!("val={:?}", val);
...
///
/// fn main() {
///     let fut = tokio_fs::xattr::remove(".", "user.myattr1").map_err(|err| {
///         eprintln!("Error: {:?}", err);
///         ()
///     });
///     tokio::run(fut);
...
///
/// fn main() {
///     let fut = tokio_fs::xattr::set(".", "user.myattr1", &[0x12, 0x34, 0x56]).map_err(|err| {
///         eprintln!("Error: {:?}", err);
///         ()
///     });
///     tokio::run(fut);
